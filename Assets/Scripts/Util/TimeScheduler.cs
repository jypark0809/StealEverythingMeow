...
public class TimeScheduler : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (lastQuitTime.Year != DateTime.Now.ToLocalTime().Year || lastQuitTime.Month != DateTime.Now.ToLocalTime().Month || lastQuitTime.Day != DateTime.Now.ToLocalTime().Day)
        {
            InitData();
        }
    ...
    public void SaveLastQuitTime()
    {
        ...
        Debug.Log($"SaveLastQuitTime : {PlayerPrefs.HasKey("LastQuitTime")}");
    }
